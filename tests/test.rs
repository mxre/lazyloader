...
fn model_new_row() {
    let e = Env::new().unwrap();
    let mut lp = Problem::new(&e, "test").unwrap();
    lp.new_row(1.0, cplex::model::Sense::LessThan, None).unwrap();
...
fn model_new_col() {
    let e = Env::new().unwrap();
    let mut lp = Problem::new(&e, "test").unwrap();
    lp.new_col(1.0, 0.0, 1.0, cplex::model::VarType::Binary, Some("x(0)")).unwrap();
...
fn write_model() {
    ...
    let mut lp = Problem::new(&e, "test").unwrap();
    lp.new_col(1.0, 0.0, 1.0, cplex::model::VarType::Binary, Some("x(0)")).unwrap();
    lp.new_row(1.5, cplex::model::Sense::LessThan, None).unwrap();
