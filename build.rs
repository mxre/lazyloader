...
fn main() {
	...
	let cpxpath = PathBuf::from(env::var(CPLEX_ENVIRONMENT_VAR).unwrap_or(String::from(CPLEX_DEFAULT)));
    ...
    let target = PathBuf::from(env::var("OUT_DIR").unwrap()).join(library.file_name().unwrap());
    if !target.exists() {
